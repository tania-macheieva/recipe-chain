...
class RecipeController {
  ...
  // GET /api/recipes/:id - Отримати один рецепт
  ...
  static async createRecipe(req, res) {
    try {
      const { title, description, ingredients, instructions, parent_id, author, changes } = req.body;
      ...
      // Перевірка чи існує parent рецепт
      if (parent_id) {
      ...
      res.status(201).json({
        success: true,
        data: newRecipe,
        message: parent_id ? 'Recipe forked successfully' : 'Recipe created successfully'
      });
    } catch (error) {
  ...
  // POST /api/recipes/:id/fork - Форкнути рецепт
  static async forkRecipe(req, res) {
    try {
      const { id } = req.params;
      const { title, description, ingredients, instructions, author, changes } = req.body;
      
      // Перевірка чи існує оригінальний рецепт
  ...
  // GET /api/recipes/:id/chain - Отримати ланцюжок предків
  ...
  // GET /api/recipes/:id/children - Отримати всі форки рецепту
  ...
  // GET /api/recipes/:id/tree - Отримати повне дерево
